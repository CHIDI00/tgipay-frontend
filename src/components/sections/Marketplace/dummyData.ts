export const topSellingProducts = [
  {
    id: "1",
    name: "Classic Minimalist Watch with Leather Strap",
    store: "TimeKeepers Official",
    price: "₦25,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCDlyrqnA6W9CkbZ8j61dJ9oAi3lLbd5FgxMV-Pet36FTL1NJ0AxxpEuyQYEH3ELW3Oih9BdZi1oahgbHm7t7mMEy3EEt3E5zbshp3Lgpw-t92Aiz76A8W9QldlxQVld-lANDCqRwsbMTezRlDxTV-gewgHp6wvxzU_Ys-9qWs7tQ8AJnRDnEFXl4roZ2YxJ4kPzbG7JEepheM_vBUW5Je-qYeh85Kb9_dQmC1wuZxkqt0S4Gd8guX-2Bc95pPvKXcqBjluWLX-yj-a",
    rating: 4,
    reviews: 42,
    hasFavorite: true,
    description:
      "Elevate your style with this classic minimalist watch featuring a genuine leather strap, precise quartz movement, and a scratch-resistant sapphire crystal face. Perfect for both formal and casual settings.",
  },
  {
    id: "2",
    name: "Wireless Noise Cancelling Headphones",
    store: "Audio Hub",
    price: "₦55,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBkeorhpHUFAtUNrPsTBgQQW44TBqri-MXslobjWqs4Xa6Sk6PBDn6caB99FRUY2yQf8hK0toM5eiEpwQsoCWXwQy7n9uLlbbp0bo-Mt_OmAEx5lQO4Kiqbbz420r2e2bYnp9nFGWr9nLe5TPeqsmrzx7qiUfTru9aynHDLZ6QkzIEGXN9vWodjyzzeRjkEfxPpgVws8EFUfX9yx1UjhQQS3GCs-BIc_stZDGvJRS4SpsVOxaS8i4kOZNh6FduqdZWV4iZfyRbQ6glG",
    rating: 5,
    reviews: 156,
    hasFavorite: false,
    description:
      "Immerse yourself in rich, high-fidelity sound with these premium wireless headphones. Features active noise cancellation, over-ear comfort, and an impressive 30-hour battery life for all-day listening.",
  },
  {
    id: "3",
    name: "Skincare Essential Set - 3 Pieces",
    store: "Glow Beauty",
    price: "₦18,500",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuByi61P6r-a0R0N7bH3dKhw65V2s6PndwA3I-k2xGz8IuQv3w2Z-7XJbH8v9UjD1R6X2o4L_N1e-3c2L6L0xXlZ8V6Xv7X3H_M9P5a8_S1o0Y_V1lq5-7_S6-8g-7T9I9xZ-7_S3h4m9Vq_r5d4P9q7-9y2T-4o7T9L-2w3N5q8n6G3q0M9P-4X0d9_C8D9x6C-3d5R3a0z6O1z2",
    rating: 4.5,
    reviews: 89,
    hasFavorite: false,
    description:
      "Revitalize your skin with this complete 3-piece essential set. Includes a gentle cleanser, a hydrating toner, and a nourishing moisturizer formulated with natural ingredients to leave your skin glowing and radiant.",
  },
  {
    id: "4",
    name: "Smart Fitness Tracker Band",
    store: "TechGear",
    price: "₦12,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuA2F-9x4q_k5Q2D5v-2n_j_U6c_T5r1X4d3E_c6M-U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U9U",
    rating: 4,
    reviews: 210,
    hasFavorite: false,
    description:
      "Achieve your health goals with this sleek smart fitness tracker. Monitors heart rate, tracks steps, analyzes sleep patterns, and provides real-time notifications right on your wrist. Water-resistant and durable.",
  },
  {
    id: "5",
    name: "Men's Casual Sneakers",
    store: "Urban Kicks",
    price: "₦35,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCqLhw5HFc4wg57S9q1wJFQmFg6Ndz5_O4bpXnD8nxvuoeJit0vaGhRVSC-USLeyR6FyRSk-O_pR_MM7x1Z-mgK6RAzRwvsywVeRO8_nR4ANMwcOym4u_rGw-cpW924qwBfkKnx_MoU1ucavj2af8Qa55VDUFYpeedco4OqWGRm78vkJ5mdxVi6BqwBsYzR2EfS2os1egTBQ77_6BiOBOf9ct24h9apcwXdteQd4J9pzZWFKfhmJGd8Sat1yp1rwwL8GZCIad2sHiEC",
    rating: 5,
    reviews: 67,
    hasFavorite: false,
    description:
      "Step out in comfort and style with these versatile men's casual sneakers. Crafted with breathable materials, a cushioned insole for all-day support, and a non-slip rubber outsole for superior traction.",
  },
];

export const recommendedProducts = [
  {
    id: "6",
    name: "Ergonomic Mesh Office Chair - High Back",
    store: "Home Office Hub",
    price: "₦78,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDjeXtb2xxDHySxATLKUadKVEprJmm6ncwvPoO9IpffBXm2OKnUOuAzG8YUvRHuUV6cN8iAKoEJpHMYOyXESffXUEfDBpNbFRk8cUhGHx2nur9TRe9hfPZY5CLJD4RCQFKt_wyU5hnqAk4xzfg_VhftDeyIZvuZU9OanrPvQzv1MvpudT4GXF3heVnPvv_AfAbgrOlgn8rUGZovd4RU9hYPPUdqyQ8Dv6KNAg69zSGlVXJaFieLi8rjYWVk99BS3nIqOJxFqZhDyKFm",
    rating: 4,
    reviews: 0,
    hasFavorite: false,
    description:
      "Improve your posture and work comfortably with this ergonomic high-back office chair. Features an adjustable headrest, lumbar support, breathable mesh material, and smooth-rolling casters.",
  },
  {
    id: "7",
    name: '50" Smart 4K Ultra HD LED TV',
    store: "Vision Direct",
    price: "₦345,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCFuDC79sWXJYxmTs3oy1aASvOa_92UZa3F3MmCbhVdvTdPtTRq7kGY9p2gXmzeEDW14D53210a5q27TMGzU_jPdQ5YyG32GMn7kJA0cXjvF9Oir3VfZImwMMwHiqxHUArwbRkLy--eAbKAMAlsOYqwgx7O1i-acQLNwFIOSNu3bwCBsi1iGjUSwm-IFf8NYFCqo7CS08Wzg8gmdQB6vRMVJZAuBt6hFNSPKZcKnwkuDYWRgXvjbws-52comO9v40wwHsoPTmMac2w-",
    rating: 5,
    reviews: 0,
    hasFavorite: false,
    description:
      "Transform your living room into a home theater with this stunning 50-inch 4K Ultra HD LED TV. Enjoy vibrant colors, deep contrasts, and built-in smart features that let you stream your favorite apps seamlessly.",
  },
  {
    id: "8",
    name: "Hi-Fi Wired Earbuds with Mic",
    store: "Sound Masters",
    price: "₦12,500",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuD6NPCu2uN-B5x7DEm8ePggNNPamqQ9p7kKED50yiymQX3_eAtDJdtWSKmsBkxVi4l_TFWVZyG-uy0wTtFnlI-Z1FOcaonvOuaKkfQweuh4plLMae8-g-2MyHVNMAbZ6tDiSVzSvs16z9T9_zv0Ew414Bt8k4oVv3f9O_Ecwd_gFUV78QEfMmQ5CHlkbZ8ptsbninWAZlDYHr448iOJ5hSIz50OPhtKJuzrp2EawkssB3nBqJavNye7VGKyre1L_mufRVU8JsjuGReh",
    rating: 4,
    reviews: 0,
    hasFavorite: false,
    description:
      "Experience crystal-clear audio with these high-fidelity wired earbuds. Designed with an ergonomic fit for noise isolation, deep bass drivers, and a convenient inline microphone for hands-free calls.",
  },
  {
    id: "9",
    name: "Premium Cotton T-Shirt - Pack of 3",
    store: "Everyday Wear",
    price: "₦15,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAN6emxLx_7iIzdZR2h7jbLB1fQSmtKtxC6T4muoxltf0FDLiC1UN5QTZQ-fBF5FCHC2mEQsysygA87Nu4pPc9zls1MhbMPiUPu641NWeUWKcS9dCdQ4Ikyo1TGCmychXXoEnwhGnBbkDljI9oGEIWvL7qN0NhUo8IJBnQ37OiSjPUHP2BV-g15MbbvkugiDpQs-Or7G7ZsKadNgB6EI6I0RYN_W9l5-O0oeGewh9KyIoYij5dqxK17bFU9bXjU2tsp5GnegVRfl47J",
    rating: 4,
    reviews: 0,
    hasFavorite: false,
    description:
      "Upgrade your wardrobe essentials with this pack of 3 premium cotton t-shirts. Ultra-soft, breathable, and designed with a tailored fit that retains its shape wash after wash. Perfect for everyday wear.",
  },
  {
    id: "10",
    name: "Portable 2TB HDD - Backup Drive",
    store: "Data Store",
    price: "₦62,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuClzoUYJpAqpZRad9qdvgyGoJlC7H1hLKoBd-XAiTjuejFEW7f4yeA_Fi0xPpVHO7ubgo-zU4DUqo_j23hbqoIZOLUzGDEtPolSISRDpbrXUm0s0qJvfJOfuOpgQqEZkChpab-OKvC7dSXHKcoehuCSULmbdtQYMzGnTCdyKeulxCNMKOWnJkMpatFsJQDCGhQT9o8NVIhcf8btKnTHDUHnTmHGsKQCu0lWY3Y8N9A0wWES9u6BGM5WzMxvT2cOjg4uvfVZYfTT67go",
    rating: 5,
    reviews: 0,
    hasFavorite: false,
    description:
      "Keep your important files, photos, and documents safe with this ultra-slim 2TB portable external hard drive. Offers fast data transfer speeds with USB 3.0 compatibility and reliable plug-and-play functionality.",
  },
];

export const newlyAddedProducts = [
  {
    id: "11",
    name: "Azure Blue Parfum - 100ml",
    store: "Luxury Scents",
    price: "₦45,000",
    description:
      "Discover a sophisticated and refreshing scent with Azure Blue Parfum. This 100ml fragrance blends citrus notes with warm woody undertones, making it the perfect signature scent for any occasion.",
  },
  {
    id: "12",
    name: "Instant Film Camera - Pastel Pink",
    store: "SnapShop",
    price: "₦32,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDJLcqKf6btEQCKAcnes2uhOqPGU15jyl667WYgxaUjfkjREPc64jqrLEMGCxGvSwZrN6OqrMjBPQMMctSqa0-9GWW2nZcxZhUJ3ZHif8TCITPFhcn9y41T0O2uU2qH9zbXrnvGoMO_1-4F5UZqAWMVk91LPOn0TOdQwakDhuECGgrtnUBuN3YAmHsY-eRI1_keSyozG8W1BTX9QUmHoybM4Ax80fvbViiKBHub5QLN6pROkgLFdwioxRT5JWgQk-ffyMAMhrKzZWey",
    rating: 0,
    reviews: 0,
    hasFavorite: false,
    description:
      "Capture memories instantly with this adorable pastel pink film camera. Features built-in flash, automatic exposure measurement, and a selfie mirror to ensure perfectly framed retro-style photos every time.",
  },
  {
    id: "13",
    name: "Classic Canvas High-Tops",
    store: "Footwear Co",
    price: "₦14,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuA-Kt91NOsapNfrZEPashFcU6cQDS47PL0o_RLGQfGJEVOfRbNE-RSfQS10_zE4IzMgoavjgGoUoue6pDrkJYK0IivXZ3g_2zGXlIKqWsDk8xk-lfq4dLqFTlw2kMzWcD0y_p9ETEqpegx-FcyGPzsqFsSyw8ks60S8T7gD8Y_UUILPwWZFcC3R0Gj5KOiU98vzOmp22pTN5IetT-g5EWXQiD7w3hqmxDRgQ11XyMcS-oaj9qleSi9VR79kVA46gUYHUWxtsvowyal4",
    rating: 0,
    reviews: 0,
    hasFavorite: false,
    description:
      "Make a statement with these timeless classic canvas high-top sneakers. Featuring durable canvas uppers, a classic lace-up design, and iconic rubber toe caps for a vintage look that never goes out of style.",
  },
  {
    id: "14",
    name: '11" Professional Tablet - 128GB',
    store: "Tech Store",
    price: "₦210,000",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuA38pFnWAd2ODTJ-_yNYL5cziYegPGIvyVobvojmou-MucFuyOVz56oSs6EA2iCAS90ixCzQp8aIe82Sawgi4j-EDK-EiIIbUBHpEk5FxCDlzDaBk049NzXatQYuABIC_CAS98HNHtVRl5rQ2VkvDaATyvaAhxxCNtZ12k7c_A1G3SWnbQZ3ok5p5hDd0imRP1QbVMonn2x_OV78O9At_S-I6VJHyzATIRKxL8fTAHwzyBLFQZ7yXWqBXRa8gxM0jpLu3orIxQE2dA5",
    rating: 0,
    reviews: 0,
    hasFavorite: false,
    description:
      "Unleash your creativity and productivity with this 11-inch professional tablet. Equipped with a stunning retina display, powerful processor, 128GB storage, and all-day battery life for work or play.",
  },
  {
    id: "15",
    name: "Genuine Leather Bifold Wallet",
    store: "Leather Works",
    price: "₦7,500",
    description:
      "Carry your essentials in style with this elegant bifold wallet. Handcrafted from top-grain genuine leather, featuring multiple card slots, a clear ID window, and a spacious bill compartment in a slim profile.",
    image:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuD3HGXfKGnxllvU9maNj2HxXm8GQJ4UiiZCklpXoSUj6hnnELmr26-yXu0mVE3b99Eof8H6mhkCU8t_xFp_2VZiQzQr8Iyzm8gQCaTjBrxAGHnScNVNnRbdg0elPnlN6ft9PxMCF6G7oyRRORX5Ub17bUclBrw2-mNXLR0kImkyF9UIcRhQ1zItiH_DsyYPzGTlW-4PIO4VvP5v81YjdQenYNIMUlC_tm3gdecMnFXiIKCnPHs5bIPNCVdel7ieq5-ZQJWsm26E23xU",
    rating: 0,
    reviews: 0,
    hasFavorite: false,
  },
];
